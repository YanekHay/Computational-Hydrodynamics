...
try:
    epsilion = 1e-3
    N = 30 # For length of y[]
    M = 30 # For length of x[]
    J = 30 + 2* N**2     # For Time
    ...
    def animate_func(num):
        ...
        for i in range(N):
            for j in range(M):
                ...
                ax.scatter(x[i], y[j], T[i, j, num], color=(red, 0.1, 0.1))
    ...
    fig = plt.figure()
    ax = plt.axes(projection='3d')
    line_ani = animation.FuncAnimation(fig, animate_func, interval=60,frames=numDataPoints)

    plt.show()


    f = r"//Users/user/Desktop/animate_func2.gif"
